...
struct Stack<bool>
{
    ...
    [[nodiscard]] static bool get(lua_State* state, int index)
    {
...
struct Stack<const char*>
{
    ...
    [[nodiscard]] static const char* get(lua_State* state, int index)
    {
...
void checkArguments(lua_State* state)
{
    ...
    if constexpr (kCount > 0) {
        ...
        for (int i = 0; i < kCount; ++i) {
            if (!matched[i]) {
                luaL_error(state, "argument %d: expected %s, got %s", i + 1, typeNames[i],
...
int invokeAndPush(lua_State* state, R (*fn)(Args...), std::index_sequence<Is...>)
{
    if constexpr (std::is_void_v<R>) {
        ...
        Stack<std::decay_t<R>>::push(
            state, fn(Stack<std::decay_t<Args>>::get(state, static_cast<int>(Is) + 1)...));
        return 1;
    }
}
...
int invokeAndPushCtx(lua_State* state, R (*fn)(Ctx&, Args...), Ctx& context,
