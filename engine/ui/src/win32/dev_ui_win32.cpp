...
bool devUiPlatformMessageHook(void* windowHandle, std::uint32_t message, std::uint64_t wParam,
                              std::int64_t lParam)
{
    if (ImGui::GetCurrentContext() == nullptr) {
        return false;
    }
    if (ImGui_ImplWin32_WndProcHandler(static_cast<HWND>(windowHandle), message,
