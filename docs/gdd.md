...
**Guns are grouped, and the trigger fires a group.** Every gun a hull carries answers to one of four **fire groups**; by default they all answer to the first, so a ship nobody has set up fires everything at once, exactly as a ship with a single gun always did. The player assigns a gun to a group on the ship readout, saved with the fit, and cycles which group the trigger is on with one key while flying — a cycle that visits only the groups that actually have guns in them. This is what makes a mixed hardpoint loadout a decision rather than a compromise: the freighter that carries a mining laser and a cannon can cut a rock without spraying bolts at it, and fight without a beam draining the capacitor. An NPC has no console and no selection — it fires every gun it carries.
...
### 14.2 Captains and standing orders **[v2]** — ✅ shipped as engine-plan Phase 39, 2026-09-03
...
**⚑⚑ THIS SECTION SURVIVED ITS RE-READ INTACT, WHICH IS RARE ENOUGH TO SAY (spec'd 2026-09-03), AND IT SHIPPED AS WRITTEN (2026-09-04).** ⚑⚑⚑ **What the build added to the promise above: the composition posts ONE GUARD PER THREE MEMBERS, capped at three** - `rollHeldFleetRaid`'s own ceiling on hulls present, so a fourth guard has nothing to shoot at - and **there is no cap on a fleet's size**, because the hull-axis instrument measured 0.028 ms a hull, dead linear, with about 590 hulls needed to spend a frame. What bounds a fleet instead is the galaxy: a crew hall seats **three** and hiring empties it for good, so six captains is two halls and twelve is four. ⚑ The Crew tab therefore reports rather than refuses, and says the thing the tally hides - every hauler past the first shares **one lane to one counter**. The commander, the customisable formation and the order-resolved-by-composition are all still what Phase 40 builds. Three things were settled around it. **A fleet is a commander CAPTAIN who re-issues the five orders of §14.2**, rather than a sixth order kind or a `Fleet` object owning captains — so subordinates keep their own record, ship, cut and ledger, and Phase 39's machinery stays whole underneath. **`steerFormation` already exists but has never had a non-player consumer**: its three call sites are all the player's own hull flying a §14.1 command mode, and a captain's hull is *paced* across its system rather than steered, so a formation is only visible where hulls are close together — at the rock, at the dock, and in a fight. ⚑⚑⚑ And the one thing that moved: **a fleet's system gets a REAL fight rather than a die roll**, reversing Phase 39's ruling for exactly the systems the player has posted a fleet in. The re-read found that reversal is cheap — combat already runs per-frame in every held bubble and only the 2 Hz *decision* was scoped out (`decisions/015`, amended a third time).
