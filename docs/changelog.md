...
- The project's first dependencies: `vulkan-headers`, `volk`, `vulkan-memory-allocator`, and `glfw3`, acquired through vcpkg manifest mode against a pinned baseline, with the Vulkan SDK pinned separately as a toolchain input. All four are linked privately, which is what makes the renderer boundary a compile error rather than a convention. Reviewed in `docs/dependencies.md`.
- An absent Vulkan driver now produces a diagnostic the game writes rather than an operating-system error before the game starts. Nothing links the Vulkan import library; entry points are resolved at runtime, so a missing loader is a condition the renderer can inspect and explain.
- A capability requirement set declared as data and checked by a pure function, so devices this project does not own can be checked against it. Every device class the hardware baseline names is verified not to be excluded by the requirements — on published specifications, which is a design check on the requirement set and explicitly not a driver test.
