...
*Next: `UiContext` and the widget set, then the station screen and menu shell, then delete the provisional ImGui HUD.*

**Known gap**: kerning comes from the legacy `kern` table only, and IBM Plex ships its pairs in GPOS, which this reader does not parse — so text currently sets on raw advances. Acceptable for v1 UI copy at these sizes; revisit if headings look loose.
