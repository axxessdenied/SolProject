...
**Evidence broadened 2026-08-14, after acceptance.** The sentence above records what acceptance rested on and stays as written: at that moment the renderer could not select a physical device, so the evidence was single-device by construction. Device selection landed the same day, and the jitter, depth, LOD-continuity (both halves) and validation-output gates now pass on **both** devices present — RTX 4060 Laptop and Intel UHD Graphics (Alder Lake-P) — from the same binary, each recorded under its own name. Capability reporting closed the same day. This strengthens the decision without changing it, and changes **nothing** about the caveat below: two present devices are not the four named baseline classes, and neither of them is AMD.

**The 1.2 floor's cost is now a measured number, recorded here because this ADR is where the floor is chosen.** LunarG's `VP_LUNARG_desktop_baseline` profiles — each the intersection of a collection of real gpuinfo.org reports — sit at API **1.1.139 for 2022**, 1.2.148 for 2023, 1.2.197 for 2024, and 1.4.304 for 2026. So the widely-deployed desktop baseline was still Vulkan 1.1 as recently as the 2022 intersection, and 2023 is the first that clears this ADR's floor. The floor is not free. It is still the right choice for a title shipping well after 2023, and it is no longer an intuition about what "modern enough" means.
...
| Startup capability reporting and graceful rejection with actionable diagnostics | **Met.** The renderer enumerates loader, device version, features, extensions, formats, queues, limits and memory, and rejects unsupported devices by named reason with a negative control proving the rejection path fires. **Reconciled 2026-08-14, and the gate closed on that basis by user decision.** A second profile family was added from the pinned SDK's `VP_LUNARG_desktop_baseline.json` — four intersections of real gpuinfo.org report collections, at a recorded SHA-256, spanning Vulkan 1.1 to 1.4. An intersection is conservative, so a requirement it satisfies is satisfied by every device in its collection, and the 2022 profile is *required to be rejected*, so both paths are exercised with real-derived data. The hand-authored four remain one assertion, not four. **Residual:** the four *named* classes are still not individually verified, since vulkan.gpuinfo.org is unreachable from the project environment; that bears on baseline-class support claims, which this ADR does not make, and is tracked in `docs/project_status.md` |
