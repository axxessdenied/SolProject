...
**Evidence broadened 2026-08-14, after acceptance.** The sentence above records what acceptance rested on and stays as written: at that moment the renderer could not select a physical device, so the evidence was single-device by construction. Device selection landed the same day, and the jitter, depth, LOD-continuity (both halves) and validation-output gates now pass on **both** devices present — RTX 4060 Laptop and Intel UHD Graphics (Alder Lake-P) — from the same binary, each recorded under its own name. This strengthens the decision without changing it, and changes **nothing** about the caveat below: two present devices are not the four named baseline classes, and neither of them is AMD.
