...
void fillRemoteMarkers(const SpaceWorld& world,
                       std::uint32_t system,
                       ...
                       std::vector<ui::MapMarkerRow>& markerRows)
{
    ...
    for (std::uint32_t i = 0; i < spec.stations.size(); ++i) {
        ...
        if (survey.stationIdentified(system, i)) {
            ...
            add(Kind::Signal, anonymousContactName(i), spec.stations[i].position, "unscanned", false, 0);
        }
    }
    for (std::uint32_t i = 0; i < spec.gates.size(); ++i) {
        ...
        if (survey.gateIdentified(system, i)) {
            ...
            add(Kind::Gate,
                "Gate: " + galaxy.systems[spec.gates[i].toSystem].name,
                spec.gates[i].position,
                {},
                true,
                0);
        } else {
            add(Kind::Signal,
                anonymousContactName(spec.stations.size() + i),
                spec.gates[i].position,
                "unscanned",
                false,
                0);
    ...
    for (std::uint32_t i = 0; i < spec.planets.size(); ++i) {
        ...
        add(Kind::Planet, planet.name, planet.position, scanned ? "scanned" : "unscanned", scanned, 0);
    }
    {
    ...
    for (std::uint32_t i = 0; i < signals.size(); ++i) {
        ...
        add(Kind::Signal,
            signalTargetName(signals[i].kind, resolved, survey.signalEmptied(system, i), contactBase + i),
            signals[i].position,
            resolved ? "scanned" : "unscanned",
            resolved,
            0);
    }
    ...
    if (objective != nullptr && objective->kind == sim::ObjectiveKind::FlyTo && objective->system == system) {
...
void fillMapTail(const SpaceWorld& world,
                 std::deque<std::string>& text,
                 ui::MapPanel& panel,
                 ...
                 const std::vector<ui::MapMarkerRow>& markerRows)
{
    ...
    panel.knownSummary = store(text,
                               std::to_string(known) + " of " + std::to_string(galaxy.systems.size()) +
...
void fillMapPanel(const SpaceWorld& world,
                  std::deque<std::string>& text,
                  ui::MapPanel& panel,
                  std::vector<ui::MapSystemRow>& systemRows,
                  std::vector<ui::MapLaneRow>& laneRows,
                  std::vector<ui::MapMarkerRow>& markerRows)
{
    text.clear();
    systemRows.clear();
    ...
    if (tradeCommodity >= 0) {
        ...
        for (const sim::MarketMemory& memory : survey.marketMemory()) {
            if (memory.market >= world.economy().markets().size() || commodity >= memory.prices.size()) {
                continue;
            }
            ...
            if (systemHasPrice[system] == 0 || price > systemPrice[system]) {
                ...
                systemStale[system] = survey.isStale(memory, world.worldSeconds()) ? 1u : 0u;
            }
    ...
    const sim::MissionObjective* objective = world.trackedObjective();
    const std::uint32_t objectiveSystem = objective != nullptr ? objective->system : sim::kAnySystem;
    ...
    for (std::uint32_t i = 0; i < galaxy.systems.size(); ++i) {
        ...
        if (visited && world.factionSim().contested(i) && contest.attacker < world.factions().size()) {
            row.contested = true;
        ...
        if (!visited) {
            row.detail = store(text, spec.name + ": charted only - fly there to survey it" + objectiveNote);
        } else {
    ...
    for (const sim::GateLink& link : galaxy.links) {
        ...
        laneRows.push_back(
            {.from = static_cast<int>(link.a), .to = static_cast<int>(link.b), .onRoute = onRoute});
    }
    ...
    if (panel.viewSystem >= 0 && static_cast<std::size_t>(panel.viewSystem) < galaxy.systems.size() &&
        survey.knowledge(static_cast<std::uint32_t>(panel.viewSystem)) != sim::KnowledgeState::Unknown) {
    ...
    panel.viewSystemName = galaxy.systems[viewSystem].name.c_str();
    {
        ...
        if (panel.viewIsCurrent) {
            ...
            const std::vector<std::uint32_t> hops = sim::routeBetween(galaxy, currentSystem, viewSystem);
            summary += hops.size() >= 2 ? ", " + std::to_string(hops.size() - 1) + " jump(s) away"
                                        : ", no route known";
        }
    ...
    if (!panel.viewIsCurrent) {
        ...
        const core::DVec3 remoteHub = survey.knowledge(viewSystem) >= sim::KnowledgeState::Visited
    ...
    const core::DVec3 hub =
        world.planets().empty()
            ? core::DVec3{}
            : world
                  .planets()[std::min<std::size_t>(galaxy.systems[world.currentSystemIndex()].primaryPlanet,
    ...
    for (std::size_t i = 0; i < targets.size(); ++i) {
        ...
        const bool orbital = kind == SpaceWorld::NavKind::Star || kind == SpaceWorld::NavKind::Planet;
        ...
        if (kind == SpaceWorld::NavKind::Signal || kind == SpaceWorld::NavKind::Planet ||
            ...
            for (std::uint32_t r = 0; r < rocks.size(); ++r) {
                const float remaining =
                    world.mining().unitsLeft(world.currentSystemIndex(), field, r, rocks[r].yieldUnits);
                left += remaining;
                standing += remaining > 0.0f ? 1u : 0u;
            }
            detail += " - " + std::to_string(standing) + " rocks, " + std::to_string(static_cast<int>(left)) +
                      " units";
