...
void drawMeter(DrawList& list, const Rect& box, float fraction, const Color& fill)
{
    ...
    const Rect filled = {box.min, {box.min.x + std::max(width, box.height()), box.max.y}};
    list.addRoundedRect(filled, box.height() * 0.5f, fill);
}

// A labelled bar: "HULL [=====   ] 62%". Returns nothing; the caller advances.
void drawLabelledMeter(DrawList& list,
                       const Styles& styles,
                       const Rect& row,
                       const char* label,
                       float fraction,
                       const Color& fill)
{
    ...
    list.addTextInBox(
        *styles.small, {{row.max.x - kValueWidth, row.min.y}, row.max}, buffer, kTextDim, TextAlign::Right);
}

// A status pill. Returns the width it consumed, so a row of them can be laid
// out left to right without measuring twice.
float drawChip(DrawList& list,
               const Font& font,
               const FontStyleRecord& style,
               Vec2 topLeft,
               float height,
               const char* text,
               const Color& color)
...
void drawDamageFlash(DrawList& list, const Rect& view, const sol::ui::FlightHud& hud)
{
    ...
    for (int ring = 0; ring < 4; ++ring) {
        ...
        list.addRectOutline(
            {{view.min.x + inset, view.min.y + inset}, {view.max.x - inset, view.max.y - inset}},
            kReticleHit.withAlpha(alpha),
            10.0f);
    }
}
...
void drawReticle(DrawList& list, Vec2 center, const sol::ui::FlightHud& hud)
{
    ...
    const auto shieldArc = [&](float fraction, bool fore) {
        ...
        list.addArc(center,
                    24.0f,
                    centerAngle - kFullSpan,
                    centerAngle + kFullSpan,
                    kShield.withAlpha(0.20f),
                    2.5f,
                    24);
        const float half = kFullSpan * clamp01(fraction);
        if (half > 0.0f) {
            list.addArc(
                center, 24.0f, centerAngle - half, centerAngle + half, kShield.withAlpha(0.82f), 2.5f, 24);
        }
    };
...
void drawLeadMarker(DrawList& list, Vec2 center, float focal, const sol::ui::FlightHud& hud)
{
    ...
    list.addLine(
        {lead.position.x - 9.0f, lead.position.y}, {lead.position.x - 5.0f, lead.position.y}, kLead, 1.5f);
    list.addLine(
        {lead.position.x + 5.0f, lead.position.y}, {lead.position.x + 9.0f, lead.position.y}, kLead, 1.5f);
}
...
void drawTargetMarker(DrawList& list,
                      const Styles& styles,
                      Vec2 center,
                      const Rect& view,
                      float focal,
                      ...
                      const sol::ui::FlightHud& hud)
{
    ...
    if (cockpit) {
        ...
        const Vec2 viewCentre = {(view.min.x + view.max.x) * 0.5f, (view.min.y + view.max.y) * 0.5f};
    ...
    list.addTriangle(
        tip, {back.x + side.x, back.y + side.y}, {back.x - side.x, back.y - side.y}, kTargetMark);
}
...
void drawFlightPanel(DrawList& list,
                     const Font& font,
                     const Styles& styles,
                     Vec2 anchor,
                     const sol::ui::FlightHud& hud,
                     Rect& panelOut)
{
    const Rect panel = {{anchor.x, anchor.y - kFlightPanelHeight}, {anchor.x + kFlightPanelWidth, anchor.y}};
    panelOut = panel;
    ...
    list.addTextInBox(*styles.small,
                      {{left, y}, {right, y + kRowHeight}},
                      hud.cameraMode,
                      kTextDim.withAlpha(0.7f),
                      TextAlign::Right);
    y += kRowHeight + 4.0f;
...
void drawPowerPanel(DrawList& list, const Styles& styles, Vec2 anchor, const sol::ui::FlightHud& hud)
{
    ...
    for (const auto& bar : bars) {
        ...
        list.addTextInBox(*styles.small, {row.min, {left + kLabelWidth, row.max.y}}, bar.label, kTextDim);
    ...
    drawMeter(list,
              {{left + kLabelWidth + 4.0f, meterY}, {right - 44.0f, meterY + kMeterHeight}},
              hud.weaponCharge,
              kPipWeapons);
    char buffer[32] = {};
...
void drawTargetReadout(DrawList& list,
                       const Styles& styles,
                       const Rect& panelArea,
                       const sol::ui::FlightHud& hud,
                       Rect& panelOut)
{
    ...
    panelOut = panel;
    list.addRoundedRect(panel, 8.0f, kPanel);
    list.addRoundedRect({{panel.max.x - 3.0f, panel.min.y}, {panel.max.x, panel.max.y}}, 1.5f, kTargetMark);
    list.pushClip({{panel.min.x + kPadding, panel.min.y}, {panel.max.x - kPadding, panel.max.y}});
    ...
    if (showFaction) {
        ...
        list.addTextInBox(
            *styles.small, {{left, y}, {right, y + 20.0f}}, buffer, attitudeColor(hud.targetAttitude));
        y += 20.0f;
    }
    ...
    list.addTextInBox(*styles.small,
                      {{left, y}, {right, y + 20.0f}},
                      closing,
                      hud.closingSpeedMetersPerSecond < 0.0f ? kWarning : kTextDim,
                      TextAlign::Right);
    y += 24.0f;
    ...
    list.addTextInBox(*styles.small,
                      {{left, y}, {right, y + 20.0f}},
                      eta,
                      hud.etaSeconds >= 0.0 ? kTextPrimary : kTextDim,
                      TextAlign::Right);
    y += 20.0f;

    if (hud.targetIsShip) {
        drawLabelledMeter(
            list, styles, {{left, y}, {right, y + 18.0f}}, "SH-F", hud.targetShieldFore, kShield);
        y += 18.0f;
        drawLabelledMeter(
            list, styles, {{left, y}, {right, y + 18.0f}}, "SH-A", hud.targetShieldAft, kShield);
        y += 18.0f;
...
void drawPrompts(
    DrawList& list, const Font& font, const Styles& styles, Vec2 center, const sol::ui::FlightHud& hud)
{
    struct Prompt
    {
        const char* text;
    ...
    chip(dockChip, sizeof(dockChip), hud.interactKey, hud.cleared ? berthVerb : "REQUEST DOCKING");
...
[[nodiscard]] Color radarColor(const sol::ui::RadarContact& contact)
{
    ...
    case sol::ui::RadarKind::Ship:
        ...
        case sol::ui::RadarAttitude::Hostile:
            return kHostile;
        case sol::ui::RadarAttitude::Friendly:
            return kFriendly;
        case sol::ui::RadarAttitude::Neutral:
            break;
        }
        return kNeutral;
    case sol::ui::RadarKind::Station:
        return kAccent;
    case sol::ui::RadarKind::Gate:
        return rgba(0x9E7BFFFFu);
    case sol::ui::RadarKind::Planet:
    case sol::ui::RadarKind::Star:
        return rgba(0x7E8C99FFu);
    case sol::ui::RadarKind::Signal:
        return rgba(0x63D8C4FFu);
    case sol::ui::RadarKind::Field:
        return rgba(0xC2A36BFFu);
    case sol::ui::RadarKind::Wreck:
        return rgba(0xA88C6EFFu);
    case sol::ui::RadarKind::Bookmark:
        return rgba(0xFFC850FFu);
    // Deliberately nothing like the bookmark's gold: "somewhere I chose" and
    // "somewhere I was sent" must never read alike.
    case sol::ui::RadarKind::Objective:
        return rgba(0xFF66C4FFu);
    }
    return kNeutral;
}
    ...
    DrawList& list, const Font& font, const Styles& styles, Vec2 center, const sol::ui::FlightHud& hud)
{
    ...
    list.addLine({center.x, center.y - kRadarRadius},
                 {center.x, center.y + kRadarRadius},
                 kAccent.withAlpha(0.12f),
                 1.0f);
    list.addLine({center.x - kRadarRadius, center.y},
                 {center.x + kRadarRadius, center.y},
                 kAccent.withAlpha(0.12f),
                 1.0f);

    const float range =
        sol::ui::radarRange(hud.radarRangeMeters > 0.0 ? hud.radarRangeMeters : sol::ui::kRadarRangeMeters);
    for (const sol::ui::RadarContact& contact : hud.radarContacts) {
        ...
        const Vec2 dot = sol::ui::radarDot(contact.offset, center, kRadarPlotRadius, range, kRadarStalkLimit);
        ...
        if (beyond) {
            ...
            list.addRoundedRect({{dot.x - size, dot.y - size}, {dot.x + size, dot.y + size}}, size, color);
        }
        if (contact.isTarget) {
    ...
    list.addTriangle({center.x, center.y - 5.0f},
                     {center.x - 3.5f, center.y + 4.0f},
                     {center.x + 3.5f, center.y + 4.0f},
                     kTextPrimary);
    ...
    char buffer[32] = {};
    formatDistance(hud.radarRangeMeters > 0.0 ? hud.radarRangeMeters : sol::ui::kRadarRangeMeters,
                   buffer,
                   sizeof(buffer));
...
void drawScannerPanel(DrawList& list,
                      const Styles& styles,
                      const Rect& panelArea,
                      const Rect& targetPanel,
                      const sol::ui::FlightHud& hud)
{
    ...
    list.addTextInBox(
        *styles.small, {{left, y}, {left + 90.0f, y + 20.0f}}, scanChip, ready ? kAccent : kTextDim);
    const float meterY = y + (20.0f - kMeterHeight) * 0.5f;
    drawMeter(list,
              {{left + 96.0f, meterY}, {right, meterY + kMeterHeight}},
              hud.pulseCharge,
    ...
    if (scanning) {
        ...
        char buffer[32] = {};
        std::snprintf(
            buffer, sizeof(buffer), "%.0f%%", static_cast<double>(clamp01(hud.scanProgress) * 100.0f));
        list.addTextInBox(
            *styles.small, {{right - 44.0f, y}, {right, y + 20.0f}}, buffer, kAccent, TextAlign::Right);
        y += 20.0f;
    }
    if (contacts) {
        char buffer[96] = {};
    ...
    if (prospecting) {
        ...
        list.addTextInBox(*styles.small, {{left, y}, {right - 70.0f, y + 20.0f}}, hud.prospectName, tone);
        char range[32] = {};
        std::snprintf(range, sizeof(range), "%.1f km", hud.prospectDistance / 1000.0);
        list.addTextInBox(
            *styles.small, {{right - 68.0f, y}, {right, y + 20.0f}}, range, tone, TextAlign::Right);
        y += 20.0f;
        char amount[64] = {};
        std::snprintf(amount,
                      sizeof(amount),
                      hud.prospectIsWreck ? "%.0f units aboard" : "%.0f of %.0f units left",
                      static_cast<double>(hud.prospectLeft),
                      static_cast<double>(hud.prospectTotal));
        list.addTextInBox(*styles.small, {{left, y}, {right - 46.0f, y + 20.0f}}, amount, kTextPrimary);
        if (!hud.prospectIsWreck && hud.prospectTotal > 0.0f) {
            const float meterY2 = y + (20.0f - kMeterHeight) * 0.5f;
            drawMeter(list,
                      {{right - 44.0f, meterY2}, {right, meterY2 + kMeterHeight}},
                      clamp01(hud.prospectLeft / hud.prospectTotal),
                      tone);
        }
        y += 20.0f;
    }
    if (collecting) {
        char buffer[96] = {};
        std::snprintf(
            buffer, sizeof(buffer), "+%.1f %s", static_cast<double>(hud.collectedUnits), hud.collectedName);
...
void drawMissionLine(DrawList& list,
                     const Styles& styles,
                     const Rect& flightPanel,
                     const sol::ui::FlightHud& hud)
{
...
void drawCommsPanel(DrawList& list,
                    const Styles& styles,
                    const Rect& panelArea,
                    const sol::ui::FlightHud& hud)
{
    ...
    const float width = std::min(520.0f, panelArea.width() - kTargetPanelWidth - kPadding * 2.0f);
    ...
    if (hud.cleared) {
        ...
        list.addTextInBox(*styles.small, {{left, y}, {right, y + 20.0f}}, buffer, kAccent);
        list.addTextInBox(
            *styles.small, {{left, y}, {right, y + 20.0f}}, distance, kTextPrimary, TextAlign::Right);
        y += 20.0f;
    }
    for (const sol::ui::CommsLine& line : hud.comms) {
        ...
        list.addTextInBox(*styles.small, {{left, y}, {left + kSenderWidth, y + 20.0f}}, line.from, sender);
        list.popClip();
        list.addTextInBox(
            *styles.small, {{left + kSenderWidth + 6.0f, y}, {right, y + 20.0f}}, line.text, body);
        y += 20.0f;
    }
    list.popClip();
}
...
void buildFlightUi(DrawList& list, const Font& font, Vec2 screenSize, const sol::ui::FlightHud& hud)
{
    ...
    if (frame.cockpit && frame.centreConsole.visible) {
        ...
        leftConsole.x = std::min(leftConsole.x, discLeft - kConsoleGap - kFlightPanelWidth);
