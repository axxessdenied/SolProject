...
- Define powered flight and attitude control during time warp. **Narrowed by P1a increment A3.** Warp under thrust is not a determinism problem, which is what this question originally assumed: A3 measured the local numerical regime as bit-identical across warp factors *provided the warp tick is an integer multiple of the fixed local step*. So the constraint is that offered warp factors must be quantised, not that powered warp must be forbidden. What remains open is whether powered warp is desirable for control-authority and player-agency reasons, and whether an analytic thrust arc is worth having. Attitude control under warp remains entirely open — A3 models a point mass with no attitude, torque, or control authority.
- Set the atmospheric boundary altitude at which propagation switches between drag-affected local integration and drag-free conic coast, and the handoff behavior across it (ADR 0011 fixes the model on each side but not the boundary). **The handoff behavior is settled** by A3's transition contract in [`docs/architecture.md`](../docs/architecture.md). **The altitude has a working value of 140 km**, recorded in ADR 0011's validation section, chosen as a physics-regime boundary rather than derived from a tolerance — A3 showed the tolerance-derived value is roughly 450 km, which would make the first playable's own contract orbit un-warpable. What remains open is final gameplay tuning against real ascent and reentry profiles, which belongs to P2/M5.
- Decide what the simulation should do about a **marginal sphere-of-influence capture** — a trajectory captured into an orbit tangent to a sphere boundary. A3 established that these have no well-conditioned crossing time: an arbitrarily small change of state moves a crossing arbitrarily far or removes it, so no sampling scheme reproduces the chronology across warp factors. This is a property of the ADR 0011 model rather than of any implementation, and the state tolerance still holds throughout. A game offering lunar capture will produce these trajectories, and the answer is most likely a deliberate rule about when the simulation commits to a capture rather than a numerical refinement.
- Set the **player-facing** tolerances for orbital prediction, maneuver execution, and contract acceptance. A3's tolerances are engineering tolerances met by six orders of magnitude; what a player can perceive, and what a contract should require, is a separate question that no A3 number answers.
- Decide whether encounter prediction is a first-playable feature. A3 built the machinery incidentally — sphere-of-influence crossings are found by bisection on the conic and are reproducible to the nanosecond — but predicting an encounter ahead of time is a search over future conics that A3 does not perform.
