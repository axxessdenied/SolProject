...
**Status, 2026-08-14.** Every threshold in this table now has a both-device result in increment B1 — render precision, depth behaviour, LOD continuity in **both** halves, and validation output — on the RTX 4060 and the Intel UHD, same binary, each recorded under its own name. This became possible only when the renderer gained an explicit device selection; before that it chose internally and preferred a discrete GPU, so the requirement in this row was unreachable rather than merely outstanding. An unmatched selection fails rather than falling back, which is what keeps a file's device name and its contents from diverging.

The memory clause needed one extra step that is worth naming, because it is the trap this plan exists to avoid. The Intel's reading is **flatter than the RTX 4060's on every statistic**, and a flat reading is exactly what the *withdrawn* memory claim looked like — withdrawn because its instrument could not vary. **A second device inherits none of the first device's controls**, so both were re-run on the Intel before the pass was recorded: 4 KiB/frame fails at 37 102 KiB/minute and 12.67 MiB, and the 8-byte graded control fails at 531.2 KiB/minute and 10.09 MiB against the RTX 4060's 271.7 and 6.47.

The 8-byte figure moving by 2× across two devices running the same binary is the general lesson for the CPU section below: **a control's sensitivity is a property of the run, not of the code**. Here the third term was frame rate — the Intel presented 316 689 frames against the RTX 4060's 256 954, and a per-frame leak scales with frame count. A constrained-CPU run changes the equivalent terms on the B2 side, which is why its controls are specified to run under the same constraint as the measurement they certify.
...
- Synthetic profiles representing the named baseline classes are constructed from published device data and run against the required-set declaration, so a required capability those devices lack is caught at the profile level even though no such device is present. A profile check is **not** a driver test and must never be reported as one.
  ...
  **Discharged 2026-08-14, and not in the form this bullet anticipated.** Hand-authored per-class profiles turned out to be one assertion evaluated four times — near-identical across every field the requirement consults, and written by the same hand as the requirements. What closed the gate instead was LunarG's `VP_LUNARG_desktop_baseline.json` from the pinned SDK, whose four profiles are each *"generated by the intersection of a collection of GPUInfo.org device reports"*: the same source this plan intended, pre-aggregated and offline at a recorded digest. An intersection is the stronger instrument, because a requirement it satisfies is satisfied by every device in its collection. The four span Vulkan 1.1 to 1.4, so the reject path fires on real-derived data rather than on a value chosen to fail. **Per-device reconciliation of the four named classes remains open** — vulkan.gpuinfo.org returns HTTP 403 to the project environment on every path including its documented API — and it bears on baseline-class support claims, which P1b makes none of.
...
4. **Any negative control runs under the same constraint as the measurement it certifies.** This is not a formality. B1 demonstrated it the hard way: the identical 8-byte-per-frame leak, from the identical binary, fitted 271.7 KiB/minute on one device and 531.2 on the other, because frame rate is a term in a per-frame leak's sensitivity. A control's discriminating power is a property of the run, not of the code. A B2 control demonstrated only on the unconstrained machine would certify nothing about a constrained measurement, and the arithmetic to bridge them is exactly the kind of inference this plan exists to refuse.
...
| A negative control runs under the same constraint as the measurement it certifies, and is never carried across configurations | Confirmed | Established by measurement in B1 rather than adopted as principle: the same 8-byte-per-frame leak from the same binary fitted 271.7 KiB/min on the RTX 4060 and 531.2 on the Intel UHD, because frame rate is a term in a per-frame leak's sensitivity. Discriminating power is a property of the run | B1 evidence, 2026-08-14 |
